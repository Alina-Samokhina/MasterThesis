...
class PersonData:
    ...
    def __init__(self, seq_len=32, data_dir="data/person", verbose=False):
        """Class obtaining the data of person activity dataset

        Args:
            seq_len: integer, number of observations in one sequence
        """
    ...
    def load_csv(self):
        ...
        last_millis = None
        if not os.path.isfile(f"{self.data_dir}/ConfLongDemo_JSI.txt"):
            print(f"ERROR: File '{self.data_dir}/ConfLongDemo_JSI.txt' not found")
            print("Please execute the command")
            print("source data_download.sh")

            sys.exit(-1)

        with open(f"{self.data_dir}/ConfLongDemo_JSI.txt", "r") as f:
            current_person = "A01"
